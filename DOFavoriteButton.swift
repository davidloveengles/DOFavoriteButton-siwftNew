...
public class DOFavoriteButton: UIButton {
    ...
    private func createLayers(image: UIImage!) {
        ...
        let imageFrame = CGRect.init(x: frame.size.width / 2 - frame.size.width / 4, y: frame.size.height / 2 - frame.size.height / 4, width: frame.size.width / 2, height: frame.size.height / 2)
        let imgCenterPoint = CGPoint.init(x: imageFrame.midX, y: imageFrame.midY)
        let lineFrame = CGRect.init(x: imageFrame.origin.x - imageFrame.width / 4, y: imageFrame.origin.y - imageFrame.height / 4 , width: imageFrame.width * 1.5, height: imageFrame.height * 1.5)
        ...
        for i in 0 ..< 5 {
            ...
            line.path = {
                ...
                path.move(to: CGPoint.init(x: lineFrame.midX, y: lineFrame.maxY))
                path.addLine(to: CGPoint.init(x: lineFrame.origin.x + lineFrame.width / 2, y: lineFrame.origin.y))
                return path
                }()
